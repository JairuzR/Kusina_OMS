...
@section('title', 'Edit Menu Item - KusinaOMS')
        ...
        <form action="{{ route('menu.items.update', $menuItem) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">{{ old('description', $menuItem->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" name="cost_price" value="{{ old('cost_price', $menuItem->cost_price) }}" step="0.01" min="0"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="preparation_time" value="{{ old('preparation_time', $menuItem->preparation_time) }}" min="1"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Calories</label>
                    <input type="number" name="calories" value="{{ old('calories', $menuItem->calories) }}" min="0"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
                </div>
            </div>
